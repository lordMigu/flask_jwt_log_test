...
CREATE PROCEDURE `sp_addUser` (IN `pUsername` VARCHAR(20), IN `pPassword` VARCHAR(20), IN `pFullname` VARCHAR(50))  BEGIN
    ...
    VALUES (pUsername, AES_ENCRYPT(pPassword, SHA2('B!3w6*NAt2T^%kvhUI*S^_', 512)), pFullname);
END$$
...
CREATE PROCEDURE `sp_verifyIdentity` (IN `pUsername` VARCHAR(20), IN `pPassword` VARCHAR(20))  BEGIN
	...
	AND CAST(AES_DECRYPT(USER.password, SHA2('B!3w6*NAt2T^%kvhUI*S^_', 512)) AS CHAR(30)) = pPassword;
END$$
...
-- (1, 'lordMigu3l', 0x98fb67ca8f459f49841208bd4261bceb, 'Miguel Cabrera');
